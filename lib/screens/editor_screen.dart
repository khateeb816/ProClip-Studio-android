...
class _EditorScreenState extends State<EditorScreen> {
  ...
  BoxConstraints _previewConstraints = const BoxConstraints();
  ...
  void initState() {
    ...
    _clipDurationController =
        TextEditingController(text: _clipDuration.toInt().toString());
    _clipCountController =
        TextEditingController(text: _customClipCount.toString());

    // Initialize empty settings first
    _videoSettings =
        _videos.map((file) => VideoSettings(videoPath: file.path)).toList();
    ...
    final AudioContext audioContext = AudioContext(
      ...
      android: AudioContextAndroid(
        ...
        audioFocus: AndroidAudioFocus
            .none, // Do not request focus, prevents pausing video
      ),
    );
  ...
  Future<void> _generateThumbnails() async {
    for (int i = 0; i < _videoSettings.length; i++) {
      if (!mounted) break;
      final path =
          await FFMpegService.generateThumbnail(_videoSettings[i].videoPath);
  ...
  Future<String?> _ensureHevcProxyForIndex(int index, File originalFile) async {
    ...
    try {
      ...
      if (mounted) {
        setState(() =>
            _videoError = "Optimizing HEVC Video... (will switch when ready)");
      }
      return null;
    } catch (_) {
  ...
  Future<void> _initPlayer({int index = 0, bool autoPlay = true}) async {
    try {
      final file =
          widget.videoFiles[index]; // Changed _videos to widget.videoFiles
      ...
      if (_videoSettings[index].proxyStatus == ProxyStatus.ready &&
          _videoSettings[index].proxyPath != null) {
        ...
        if (mounted)
          setState(
              () => _videoError = null); // Clear "Optimizing..." if it was set
      }
      ...
      _transformController.value = _transformController.value; // Ensure no reset from rebuild
  ...
  Future<void> _changeVideo(int index) async {
    ...
    setState(() {
      ...
      // Restore this video's settings
      final setting = _videoSettings[index];
      _aspectRatio = setting.aspectRatio;
      if (setting.transform != null) {
        _transformController.value = Matrix4.copy(setting.transform!);
      } else {
  ...
  void _saveCurrentVideoState() {
    ...
    setting.transform = Matrix4.copy(matrix);
    setting.scale = matrix.storage[0];
    setting.tx = matrix.storage[12];
    setting.ty = matrix.storage[13];
    setting.aspectRatio = _aspectRatio;
    setting.viewportSize = _viewportSize;
    if (_videoController != null && _videoController!.value.isInitialized) {
      setting.displayAR = _videoController!.value.aspectRatio;
    }

    final double videoAR =
        (setting.metadata != null && setting.metadata!.height > 0)
            ? (setting.metadata!.width / setting.metadata!.height)
            : (_videoController?.value.aspectRatio ?? 1.0);

    setting.cropRect = _getPreciseCropRect(videoAR, matrix, _viewportSize);
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color:
                              isSelected ? Colors.cyanAccent : Colors.grey[800],
  ...
  Rect _getPreciseCropRect(double videoAR, Matrix4 transform, Size viewportSize) {
    if (viewportSize.width <= 0 || viewportSize.height <= 0)
      return const Rect.fromLTWH(0, 0, 1, 1);

    final Matrix4 inverse = Matrix4.copy(transform)..invert();

    final double viewportW = viewportSize.width;
    final double viewportH = viewportSize.height;
    ...
    final Rect initialVideoRect =
        Rect.fromLTWH(offsetX, offsetY, videoW, videoH);

    // Intersection
    final Rect visibleVideoArea =
        visibleInChildSpace.intersect(initialVideoRect);
  ...
  Future<void> _startExport() async {
    ...
    // Refresh metadata for all clips if missing (essential for crop calc)
    for (int i = 0; i < _videoSettings.length; i++) {
    ...
    // Save current video state first
    _saveCurrentVideoState();

    // Ensure all videos have a viewportSize and cropRect (even if default)
    // but DO NOT overwrite those already explicitly set/panned by the user.
    for (int i = 0; i < _videoSettings.length; i++) {
       final setting = _videoSettings[i];
       
       // Calculate the video's own aspect ratio for fallback, prioritizing the true Display AR
       final double videoAR = setting.displayAR ?? ((setting.metadata != null && setting.metadata!.height > 0)
           ? (setting.metadata!.width / setting.metadata!.height)
           : 1.0);

       // If the video was never displayed, calculate its viewport based on its own ratio
       if (setting.viewportSize == Size.zero) {
         setting.viewportSize = _calculateViewportSize(setting.aspectRatio, videoAR, _previewConstraints);
       }
       
       // Always ensure cropRect is initialized for its specific transform/viewport
       if (setting.cropRect == null) {
          setting.cropRect = _getPreciseCropRect(videoAR, setting.transform ?? Matrix4.identity(), setting.viewportSize);
       }
    }
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  final res = await Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (c) => const AudioPickerScreen()));
              ...
              ListTile(
                leading:
                    const Icon(Icons.video_library, color: Colors.cyanAccent),
                title: const Text("Extract from Video",
                    style: TextStyle(color: Colors.white)),
                subtitle: const Text("Use audio from another video",
                ...
                onTap: () async {
                  ...
                  final res = await Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (c) => const VideoAudioPickerScreen()));
                  if (res != null && res is String) {
                    setState(() {
                      _audioPath = res;
                    });
              ...
              child: Stack(
                ...
                children: [
                  if (_videoSettings[index].thumbnailPath != null)
                    Image.file(File(_videoSettings[index].thumbnailPath!),
                        fit: BoxFit.cover)
                  else
                    Center(
                        child: Text("${index + 1}",
                            style: const TextStyle(
                                ...
                                color: Colors.white))),
                  if (isSelected)
                    Container(color: Colors.cyanAccent.withValues(alpha: 0.3)),
                ],
              ),
            ),
          ),
          ...
          child: Container(
            ...
            child: ClipRect(
              child: LayoutBuilder(builder: (context, constraints) {
                _previewConstraints = constraints;
                _viewportSize =
                    ...
                    Positioned.fill(
                      child: InteractiveViewer(
                        ...
                        onInteractionEnd: (details) {
                          _saveCurrentVideoState();
                          print(
                              "✋ Interaction End. Saved State: ${_transformController.value.storage}");
                        },
                        child: Center(
                          child: AspectRatio(
                ...
                if (_isAudioLoaded && _audioMode != 'original') {
                  ...
                  if (audioDur != null && audioDur.inMilliseconds > 0) {
                    final audioPos = Duration(
                        milliseconds:
                            newTime.inMilliseconds % audioDur.inMilliseconds);
              ...
              children: [
                Expanded(
                    child: _buildAudioModeButton(
                        "Mix Both", "mix", Icons.merge_type)),
                const SizedBox(width: 12),
                Expanded(
                    child: _buildAudioModeButton(
                        "Music Only", "background", Icons.music_note)),
              ],
            ),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(
                    child: _buildAudioModeButton(
                        "Video Only", "original", Icons.videocam)),
                const SizedBox(width: 12),
                Expanded(
                    child: _buildAudioModeButton(
                        "Muted", "mute_all", Icons.volume_off)),
              ],
            ),
          ],
        ),
            ...
            children: ratios.map((r) {
              ...
              return GestureDetector(
                onTap: () {
                  ...
                  _saveCurrentVideoState(); // Save ratio change immediately
                },
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        border: Border.all(
                            color:
                                isSelected ? Colors.cyanAccent : Colors.grey),
  ...
  Size _calculateViewportSize(String ratioStr, double videoAR, BoxConstraints constraints) {
    if (constraints.maxWidth == 0 || constraints.maxHeight == 0) return Size.zero;
    
    double targetAR = _getArValue(ratioStr) ?? videoAR;
